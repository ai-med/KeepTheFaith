...
from .utils import NORMALIZE, INVERSE_NORM
LOG = logging.getLogger(__name__)

BIRDS_TRAIN_TRANSFORM = transforms.Compose([
...
BIRDS_VAL_TRANSFORM = transforms.Compose([
    transforms.Resize(size=(224, 224)),
    transforms.ToTensor(),
    NORMALIZE,
    ])
...
class BIRDS(Dataset):
    ...
    def __init__(self, path: Path, is_train: bool):
        ...
        for _, x in self.df.iterrows():
                ...
                # n_gray += 1
            self._images.append(self.basepath / x.img_path)
            self._labels.append(x.label)
...
class BirdsDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_data = BIRDS(self.data_dir / f"splits/{self.fold}-train.csv", True)
        self.eval_data = BIRDS(self.data_dir / f"splits/{self.fold}-valid.csv", False)
        self.test_data = BIRDS(self.data_dir / f"splits/{self.fold}-test.csv", False)
        self.push_data = BIRDS(self.data_dir / f"splits/{self.fold}-train.csv", False)
